...
pub const DIR_LIST_REQUEST_CHARACTERISTIC_UUID: &'static str = "b6c4219e-242d-4999-a099-657bc8c7e288";
pub const DIR_LIST_RESPONSE_CHARACTERISTIC_UUID: &'static str = "f2f0a3b6-8c2e-4d6d-affd-89880b6cb844";
pub const METADATA_REQUEST_CHARACTERISTIC_UUID: &'static str = "7c603293-2e90-471f-9abd-03228b7ff1ae";
pub const METADATA_RESPONSE_CHARACTERISTIC_UUID: &'static str = "fbae61a1-9427-4274-8c1f-1f5d6b002f8d";
...
impl<'a> BleProvisioningServer<'a> {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        info!("Created service with UUID: {:?}", uuid128!(PERSONAL_CLOUD_SERVICE_UUID));
        ...
        info!("Created WiFi Config Characteristic with UUID: {:?}", uuid128!(WIFI_CONFIG_CHARACTERISTIC_UUID));
        ...
        wifi_config_characteristic.lock().on_write(move |args| {
            info!("Received data on WiFi Config characteristic: {:?}", args.recv_data());
            ...
            match serde_json::from_slice::<BleWifiConfig>(received_data) {
                Ok(wifi_config) => {
                    ...
                    let mut wifi = match wifi_arc_clone.lock() {
                        Ok(wifi) => wifi,
                        Err(e) => {
                            error!("Failed to lock WiFi mutex: {}", e);
                    ...
                    match wifi_config.mode {
                        WifiMode::Station => {
                            if let (Some(ssid), Some(password)) = (wifi_config.ssid, wifi_config.password) {
                                info!("Attempting to connect to WiFi station: {}", ssid);
                                // Call a function in wifi.rs to configure and connect
                                // Example: match crate::wifi::connect_station(&mut wifi, &ssid, &password) {
                        ...
                        WifiMode::AccessPoint => {
                            ...
                            // Example: match crate::wifi::start_ap(&mut wifi, crate::config::WIFI_SSID, crate::config::WIFI_PASSWORD) {
                ...
                Err(e) => {
                    error!("Failed to parse WiFi config data: {:?} - {}", received_data, e);
                }
            }
        });
        ...
        info!("Created WiFi Status Characteristic with UUID: {:?}", uuid128!(WIFI_STATUS_CHARACTERISTIC_UUID));

        let wifi_arc_clone = self.wifi.clone();
        wifi_status_characteristic.lock().on_read({
            let wifi_status_characteristic = wifi_status_characteristic.clone();
            move |_, _| {
                ...
                let wifi = match wifi_arc_clone.lock() {
                    ...
                    Err(e) => {
                        ...
                        wifi_status_characteristic.lock().set_value(b"Error accessing WiFi status");
                        return; // Return an error message
                    }
                };
    
                // Retrieve and format WiFi status
                match crate::wifi::get_wifi_status(&wifi) { // Assuming get_wifi_status exists
                    Ok((ip, connected)) => {
                        let status_str = format!("Status: {}, IP: {}", if connected { "Connected" } else { "Disconnected" }, ip);
                        info!("Sending WiFi status: {}", status_str);
                        wifi_status_characteristic.lock().set_value(status_str.as_bytes());
                    }
                    Err(e) => {
                        error!("Failed to get WiFi status for BLE read: {}", e);
                        wifi_status_characteristic.lock().set_value(b"Error getting WiFi status");
                    }
                }
            }
        });
        ...
        info!("Created Dir List Request Characteristic with UUID: {:?}", uuid128!(DIR_LIST_REQUEST_CHARACTERISTIC_UUID));
        ...
        dir_list_request_characteristic.lock().on_write({
            ...
            move |args| {
                info!("Received data on Dir List Request characteristic: {:?}", args.recv_data());
                let received_data = args.recv_data();
                ...
                match serde_json::from_slice::<BleDirListRequest>(received_data) {
                    Ok(dir_list_req) => {
                        ...
                        let dir_list_response_char_uuid = uuid128!(DIR_LIST_RESPONSE_CHARACTERISTIC_UUID);
    
                        esp_idf_svc::hal::task::block_on(
                            async {
                                if let Some(dir_list_response_characteristic) = service.lock().get_characteristic(dir_list_response_char_uuid).await {
                                    let response_data = b"Directory listing data..."; // Replace with actual data (serialize your directory listing)
                                    info!("Sending directory listing data ({} bytes).", response_data.len());
                                    dir_list_response_characteristic.lock().set_value(response_data).notify();
                                } else {
                ...
                Err(err) => {
                    ...
                    error!("Failed to parse Dir List Request data: {:?} - {:?}", received_data, err);
        ...
        info!("Created Dir List Response Characteristic with UUID: {:?}", uuid128!(DIR_LIST_RESPONSE_CHARACTERISTIC_UUID));
        ...
        info!("Created Metadata Request Characteristic with UUID: {:?}", uuid128!(METADATA_REQUEST_CHARACTERISTIC_UUID));

        let root_path_arc_clone = self.root_path.clone();
        // let manifest_manager_arc_clone = self.manifest_manager.clone(); // Assuming ManifestManager is used
        let service_arc = Arc::new(Mutex::new(service.clone())); // Clone service for access in closure

        metadata_request_characteristic.lock().on_write(move |args| {
            info!("Received data on Metadata Request characteristic: {:?}", args.recv_data());
            let received_data = args.recv_data();
            ...
            match serde_json::from_slice::<BleMetadataRequest>(received_data) {
                Ok(metadata_req) => {
                    ...
                    esp_idf_svc::hal::task::block_on(
                        async {
                            if let Some(metadata_response_characteristic) = service.lock().get_characteristic(metadata_response_char_uuid).await {
                                ...
                                metadata_response_characteristic.lock().set_value(response_data).notify();
                            } else {
                ...
                Err(e) => {
                    error!("Failed to parse Metadata Request data: {:?} - {}", received_data, e);
                }
            }
        });
        ...
        info!("Created Metadata Response Characteristic with UUID: {:?}", uuid128!(METADATA_RESPONSE_CHARACTERISTIC_UUID));
        ...
        info!("Created System Info Characteristic with UUID: {:?}", uuid128!(SYSTEM_INFO_CHARACTERISTIC_UUID));

        let wifi_arc_clone = self.wifi.clone();
        let root_path_arc_clone = self.root_path.clone();
        system_info_characteristic.lock().on_read({

            let system_info_characteristic = system_info_characteristic.clone();
            move |_, _| {
                ...
                let wifi = match wifi_arc_clone.lock() {
                    ...
                    Err(e) => {
                        error!("Failed to lock WiFi mutex for read: {}", e);
                        system_info_characteristic.lock().set_value(b"Error accessing resources");
                        return;
                    }
                };
                ...
                match crate::system_info::get_system_info(&wifi, &root_path) { // Assuming get_system_info exists
                    Ok(system_info) => {
                        let response = serde_json::json!(system_info).to_string();
                        info!("Sending system info: {}", response);
                        system_info_characteristic.lock().set_value(response.as_bytes());
                    }
                    Err(e) => {
                        error!("Failed to get system info for BLE read: {}", e);
                        system_info_characteristic.lock().set_value(b"Error getting system info");
                    }
                }
            }
        });
